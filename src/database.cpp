...
Database::Database(QObject *parent) : QObject(parent){
    ...
    operator_logins.append("oper");
    operator_passwords.append("oper");
...
QString Database::checkPassword(bool admin_chosen, QString username, QString password)
{
    if(admin_chosen){
        int c = -1;
        for(int i = 0; i < admin_logins.size(); ++i){
            if(admin_logins[i] == username){
                c = i;
                break;
            }
        }
...
QSqlRelationalTableModel* Database::printTable(QString table_name)
{
...
QStringListModel* Database::getTables()
{
    table_names_model->setStringList(database.tables());
    return  table_names_model;
}

QStringListModel *Database::getColumns(QString table_name)
{
    model->setTable(table_name);
...
int Database::getLastID(QString table_name)
{
...
bool Database::insertLineInto(QString table_name)
{
    QSqlQuery query(database);
...
bool Database::deleteLineFrom(QString table_name, int id)
{
    QSqlQuery query(database);
...
int Database::getID(QString table_name, int row)
{
    QSqlQuery query(database);
...
QString Database::getColumnName(QString table_name, int col)
{
    model->setTable(table_name);
    return model->headerData(col, Qt::Horizontal, Qt::DisplayRole).toString();
}

bool Database::updateField(QString table_name, int col, int ID, QString value)
{
    QSqlQuery query(database);
    QSqlQuery query_to_know_type(database);
    QString type;
...
bool Database::addColumn(QString table_name, QString column_name, QString column_type)
{
    bool res = false;
    QSqlQuery query(database);
...
bool Database::deleteColumn(QString table_name, QString column_name)
{
    model->setTable(table_name);
    int column_number = model->columnCount();
    QStringList columns;
...
bool Database::addTable(QString table_name)
{
    bool res = false;
    QSqlQuery query(database);
...
bool Database::deleteTable(QString table_name)
{
...
bool Database::renameColumn(QString table_name, QString old_name, QString new_name)
{
    bool res = false;
    QSqlQuery query(database);
...
bool Database::makeForeignKey(QString FK_table_name, QString PK_table_name)
{
